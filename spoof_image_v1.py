"""
spoof_image_v1.py — Production-ready Face Anti-Spoof Detector
==============================================================
Stack  : PyTorch (CPU/GPU) + InsightFace face detection
Model  : models/mobilenet_v3_small.pth  (MobileNetV3-Small, input 224×224)
...
# CLI:
    python spoof_image_v1.py                         # test/ folder (default)
    python spoof_image_v1.py --image foto.jpg
    python spoof_image_v1.py --folder test/
    python spoof_image_v1.py --folder test/ --benchmark
"""
...
PTH_MODEL_PATH = MODELS_DIR / "mobilenet_v3_small.pth"

# Tambahkan repo ke path agar bisa import nets.utils
...
class SpoofDetector:
    ...
    Parameters
    ----------
    pth_path     : path ke file .pth (default: models/mobilenet_v3_small.pth)
    spoof_thresh : threshold skor spoof (default 0.5)
    device       : perangkat inferensi, mis. 'cpu' atau 'cuda' (default: 'cpu')
    ...
    def __init__(
        self,
        pth_path:      str | Path  = PTH_MODEL_PATH,
        ...
        self._pth_path    = Path(pth_path)
        self._device      = torch.device(device)
        self._infer_lock  = threading.Lock()

        logger.info("Memuat PyTorch model…")
        ...
        logger.info(
            "SpoofDetector siap — model: %s | thresh: %.2f | device: %s",
            self._pth_path.name, self.spoof_thresh, self._device,
        )

    # ── Internal: build PyTorch model ──────────────────────────
    def _build_pt_model(self) -> torch.nn.Module:
        if not self._pth_path.exists():
            raise FileNotFoundError(
                f"Model .pth tidak ditemukan: {self._pth_path}\n"
                "Unduh model terlebih dahulu atau jalankan: python convert_model_onnyx.py"
            )

        from nets.utils import get_model, load_pretrain

        model = get_model("mobilenet_v3_small", num_classes=2)
        model = load_pretrain(str(self._pth_path), model)
        model.eval()
        model.to(self._device)
        logger.info("PyTorch model siap — device: %s", self._device)
        return model
    ...
    def _warmup(self) -> None:
        dummy = torch.zeros(1, 3, INPUT_SIZE, INPUT_SIZE, device=self._device)
        with torch.no_grad():
            for _ in range(3):
                self._model(dummy)
    ...
    def predict_patch(self, img_bgr: np.ndarray) -> tuple[bool, float]:
        ...
        (is_real, spoof_score)
        """
        tensor = torch.from_numpy(self._preprocess_single(img_bgr)).to(self._device)
    ...
    def predict_batch(self, patches: list[np.ndarray]) -> list[tuple[bool, float]]:
        ...
        batch_t = torch.from_numpy(batch).to(self._device)

        with self._infer_lock, torch.no_grad():
            logits = self._model(batch_t).cpu().numpy()
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Production Face Anti-Spoof Detector (PyTorch .pth)",
    ...
    parser.add_argument("--device",    type=str,   default="cpu",
                        help="Perangkat inferensi: 'cpu' atau 'cuda'")
    ...
    print(f"\n{'='*65}")
    print(f"  Production Anti-Spoof  |  {len(files)} gambar  |  model: mobilenet_v3_small.pth")
    print(f"{'='*65}")
